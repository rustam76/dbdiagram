...
import { FieldLayoutChangeEvent, type EventManager, ActiveRelationEvent } from '../../event';
...
class FieldState extends State<Field> {
	...
	override initState(context: BuildContext): void {
		...
		this.eventManager.addEventListener(ActiveRelationEvent.type, this.handleActiveRelationEvent);
	...
	override dispose(): void {
		...
		this.eventManager.dispatchEvent(new FieldRemoveEvent({ id: this.widget.field.id }));
	}
		...
		return ChangedLayoutNotifier({
			...
			child: GestureDetector({
				...
				child: Tooltip({
					...
					child: FieldContent({ field: this.widget.field, active: this.active }),
					tooltip:
