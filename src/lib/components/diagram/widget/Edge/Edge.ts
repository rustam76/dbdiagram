...
class Edge<FIELD_ID extends number = number> extends StatefulWidget {
	...
	constructor(props: {
		...
		super(`${props.from.id}-${props.to.id}`);
	...
	override initState(context: BuildContext) {
		...
		context.scheduler.addPostFrameCallbacks(() => {
			this.setState(() => {
				...
				const fromKey = fields[this.widget.from.id];
				const toKey = fields[this.widget.to.id];
				if (fromKey == null || toKey == null) {
					return;
				}
				this.from = this.controller.getRect(fromKey);
				this.to = this.controller.getRect(toKey);
			});
		});
	}
		...
		return CustomPaint({
			painter: {
				...
				paint: ({ path }) => {
					...
					path.setAttribute('stroke', this.active ? 'white' : 'lightgrey');
