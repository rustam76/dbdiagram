...
import { LegacyNodeEvent, type EventManager } from '$lib/components/diagram/event';
...
class Node extends StatefulWidget {
	...
	constructor({ table, onLegacy }: { table: Table; onLegacy: (key: GlobalKey) => void }) {
		super(table.id);
		this.table = table;
...
class NodeState extends State<Node> {
	...
	handleDragUpdate = ({ movement }: { delta: Offset; movement: Offset }): void => {
		this.setState(() => {
	...
	handleDragEnd = (): void => {
		...
		this.eventManager.dispatchEvent(
			new LegacyNodeEvent({ tableName: this.widget.table.name, key: this.nodeKey })
		);
	};
