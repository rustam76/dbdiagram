...
class BaseNodeState extends State<baseNode> {
	...
	override dispose(): void {
		super.dispose();
		this.eventManager.dispatchEvent(new NodeRemoveEvent({ tableName: this.widget.table.name }));
	}

	private dispatchActiveEvent(active: boolean) {
		const relatedFields = this.controller.getRelatedFieldIdsByTableId(this.widget.table.id);
		relatedFields.forEach((field) => {
			this.eventManager.dispatchEvent(new ActiveRelationEvent({ field, active }));
		});
	}

	handleMouseEnter = () => {
		if (this.controller.isDragging && !this.controller.isDraggingTarget(this.widget.table.name))
			return;
	...
	handleChangeSize = () => {
		this.eventManager.dispatchEvent(
			new NodeLayoutChangeEvent({ tableName: this.widget.table.name, key: this.key })
		);
	};
