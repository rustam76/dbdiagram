...
class AlignLinerState extends State<AlignLiner> {
	...
	initState(context: BuildContext): void {
		this.addPostFrameCallback = context.scheduler.addPostFrameCallbacks.bind(context.scheduler);
		this.controller = DiagramControllerProvider.of(context);
		this.eventManager = EventManagerProvider.of(context);

		this.eventManager.addEventListener(NodeTranslationEvent.type, this.handleNodeTranslation);
		this.eventManager.addEventListener(NodeRemoveEvent.type, this.handleNodeRemove);
		this.eventManager.addEventListener(NodeInsertEvent.type, this.handleNodeInsert);
		this.eventManager.addEventListener(NodeTranslationEndEvent.type, this.handleNodeTranslationEnd);
		this.eventManager.addEventListener(NodeLayoutChangeEvent.type, this.handleNodeLayoutChange);
	}

	dispose(): void {
		super.dispose();
		this.eventManager.removeEventListener(NodeLayoutChangeEvent.type, this.handleNodeLayoutChange);
		this.eventManager.removeEventListener(NodeTranslationEvent.type, this.handleNodeTranslation);
		this.eventManager.removeEventListener(NodeRemoveEvent.type, this.handleNodeRemove);
		this.eventManager.removeEventListener(NodeInsertEvent.type, this.handleNodeInsert);
	...
	handleNodeInsert = (e: NodeInsertEvent) => {
		 ...
		 * 이는 flitter에서 같은 키를 가진 위젯의 lazy dispose를 구현하지 않아서 발생하는 문제이다.
		 ...
		 * setTimeout은 flitter의 해당 이슈가 해결되면 사라질 예정이다.
	...
	private findRect(tableName: string) {
		...
		if (result == null) throw new Error(`Cannot find rect for ${tableName} on AlignLiner`);
	...
	override build() {
		return ZIndex({
			...
			child: Stack({
				children: this.lines.map((line) => Positioned.fill({ child: Line({ line }) }))
			})
		});
	}
}
